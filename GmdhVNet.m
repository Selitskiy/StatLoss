classdef GmdhVNet < BaseNetV & MLPInputNetV
    ...
    methods
        function net = GmdhVNet(m_in, n_out, accTarget1, accTarget2, accRel1, accRel2, lMax, freeze, mb_size, ini_rate, max_epoch)
        ...
        function [net, X, Y, B, k_ob] = TrainTensors(net, M, m_in, n_out, l_sess, n_sess, norm_fl)
            ...
            net.max_neuro1 = floor(net.k_hid1/net.n_oute); % Based on non-V k_hid
            net.max_neuro2 = floor(net.k_hid2/net.n_oute); %floor(net.k_hid2/2);
        end
...
'MaxEpochs',floor(net.max_epoch/2));

sOptions2 = trainingOptions('adam', ...
'ExecutionEnvironment','parallel',...
'Shuffle', 'every-epoch',...
'MiniBatchSize', net.mb_size, ...
'InitialLearnRate',net.ini_rate*3, ...
'MaxEpochs',floor(net.max_epoch/2));
...
'MaxEpochs',floor(net.max_epoch/2));  
    ...
    oLayers = [
        featureInputLayer(net.m_ine, 'Name', prevLayerName)
    ];
    ...
    %max_neuro1 = floor(net.k_hid1/2); % Based on non-V k_hid
    %max_neuro2 = floor(net.k_hid2/2); 
    min_neuro1 = 0;
    min_neuro2 = 0;
    ...
    [cgraph, regNet, ll, k_hid1_real, curGMDHLayerName, curGMDHRegressionName] =... 
        gmdhLayerGrowN(cgraph, prevLayerName, sOptions, X, Y, net.m_ine, i, net.n_oute, ll,...
    ...
    if(k_hid1_real > 1)
        ...
        [cgraph, regNet, ll, k_hid2_real, curGMDHLayerName, curGMDHRegressionName] =... 
            gmdhLayerGrowN(cgraph, prevLayerName, sOptions2, X, Y, k_hid1_real, i, net.n_oute, ll,...
        ...
        if(k_hid2_real > 1)
            ...
            fullyConnectedOutLayer = fullyConnectedLayer(net.n_oute,'Name',fullyConnectedOutLayerName);
    ...
    regressionLayerLast = vRegression(regressionLayerName, net.n_outv);
