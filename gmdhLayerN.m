classdef gmdhLayerN < nnet.layer.Layer
    ...
    methods
        function layer = gmdhLayerN(name, numInChannels, numOutProd)
            ...
            bound = sqrt(6 ./ (layer.numOutChannels(1) * numOutProd + layer.numInChannels(1)));
            layer.W = layer.M .* (bound * (2. * rand([layer.numOutChannels(1) * numOutProd, layer.numInChannels(1)], 'single') - 1.));

            %layer.W0 = rand([layer.numOutChannels(1) * numOutProd, 1]);
            layer.W0 = zeros([layer.numOutChannels(1) * numOutProd, 1]);
        ...
        function layer = pruneNACC(layer, n_max1, n_min1, acc, accRel, MSE)
            acc_init = acc;
            n_max = repmat(n_max1, [layer.numOutProd, 1]);
            n_min = repmat(n_min1, [layer.numOutProd, 1]);
            IGlob = zeros([sum(n_max,1), 1]);
            ...
            for i = 1:layer.numOutProd
                ...
                offset = offset + layer.numOutChannels(i);
                layer.numOutChannels(i) = n_max(i);
            end
